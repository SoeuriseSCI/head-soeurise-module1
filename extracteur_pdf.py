...
NE retourne QUE le JSON, sans texte avant ou après."""
    ...
    def _construire_prompt_unifie(self, sections_ordonnees: List[Dict]) -> str:
        ...
        prompt_parts.extend([
            ...
            "",
            "RÈGLE ABSOLUE - EXTRACTION EXHAUSTIVE:",
            "- Extrais TOUTES les opérations présentes dans le document, sans exception",
            "- N'omets AUCUNE ligne d'opération, même si elle semble répétitive",
            "- Si une opération apparaît plusieurs fois (ex: mensuelle), extrais-la CHAQUE fois",
            "- Continue jusqu'à la dernière page, ne t'arrête pas avant la fin",
            "",
            "À IGNORER (uniquement):",
            "- En-têtes de colonnes (Date, Libellé, Débit, Crédit, etc.)",
            "- Lignes de totaux et sous-totaux",
            "- Soldes d'ouverture et de clôture (ANCIEN SOLDE, NOUVEAU SOLDE, etc.)",
            "",
